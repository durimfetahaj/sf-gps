...
import { ArrowLeft, TruckIcon, User, UserX } from "lucide-react";
import { useRouter } from "next/navigation";

type WorkerWithVehicles = Worker & {
  vehicles: Vehicle[] | null;
};

export default function Client({ worker }: { worker: WorkerWithVehicles }) {
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              <span className="text-lg">{worker.email || "Unknown"}</span>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              {worker.vehicles ? (
                <Badge variant="outline" className="px-3 py-1.5">
                  ...
                  <span>{worker.vehicles[0].licensePlate}</span>
                </Badge>
              ) : (
                <Badge variant="outline" className="px-3 py-1.5">
